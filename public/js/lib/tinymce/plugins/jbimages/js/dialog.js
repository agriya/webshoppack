...
var jbImagesDialog = {
	...
	uploadFinish : function(result) {
		...
		else
		{
			...
			var pathname = location.pathname.split('/js/');
			var site_url = location.protocol + "//" + location.hostname + (location.port && ":" + location.port) + pathname[0];
			tinyMCEPopup.editor.execCommand('mceInsertContent', false, '<img src="' + site_url + result.filename +'" />');
			tinyMCEPopup.close();
		}
	}
