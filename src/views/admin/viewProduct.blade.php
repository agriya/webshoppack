...
@section('content')
    ...
    <div class="message-navbar mb20">
		<h1 class="admin-title blue bigger-150">{{{ $product_title }}}</h1>
		<a href="{{ Url::to(Config::get('webshoppack::admin_uri').'/list') }}" class="btn btn-info btn-xs pull-right"><i class="icon-plus-sign"></i> {{trans('webshoppack::admin/productList.product_list_title')}}</a>
	</div>
    @if($d_arr['alert_msg'] != '')
    ...
    <div class="row viewpro-store">
        ...
        {{-- Right block start --}}
            @include('webshoppack::admin.viewProductRightBlock')
        {{-- Right block end --}}
    </div>
