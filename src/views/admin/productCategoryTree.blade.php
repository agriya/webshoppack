...
@section('content')
	<div class="message-navbar mb20 mt10">
		...
		<h1 class="admin-title blue bigger-150">{{ trans('webshoppack::admin/manageCategory.manage_product_catalog_title') }}</h1>
	</div>
	...
	<div id="dialog-category-delete-confirm" title="" style="display:none;">
		...
		<span class="show ml15">{{ trans('webshoppack::admin/manageCategory.delete-category.delete_category_confirm') }}</span>
	</div>
	...
	<div class="widget-box">
		<div class="widget-header">
			<h4 class="clsRootCategory lighter smaller">{{ trans('webshoppack::admin/manageCategory.root_category') }}</h4>
		</div>
	...
	@include('webshoppack::admin.manageProductCatalogTabs')
@stop
@section('script_content')
	<script src="{{ URL::asset('packages/agriya/webshoppack/js/formbuilder/js/jquery.form.js') }}"></script>
	<script src="{{ URL::asset('packages/agriya/webshoppack/js/lib/jQuery_plugins/jquery.cookie.js') }}"></script>
	<script src="{{ URL::asset('packages/agriya/webshoppack/js/lib/jQuery_plugins/jquery.hotkeys.js') }}"></script>
	<script src="{{ URL::asset('packages/agriya/webshoppack/js/lib/jQuery_plugins/jquery.jstree.js') }}"></script>
	...
	<script type="text/javascript">
				...
				.jstree({
					...
					"json_data" : {
						...
						"ajax" : {
							...
							"url" : "{{URL::action('Agriya\Webshoppack\AdminManageProductCatalogController@getCategoryTreeDetails')}}",
						...
						"items" : function(node){
							var obj = {
								"add_sub_cat" : {
									...
									"label" : "{{ trans('webshoppack::admin/manageCategory.add_sub_category') }}",
									"action" : function (obj) {
								...
								"remove_cat" : {
									...
									"icon" : false,
									"separator_after" : false,
									"label" : "{{ trans('webshoppack::admin/manageCategory.delete_category') }}",
									"action" : function (obj) {
										var category_node = obj.attr('category_id');
		   								category_id = category_node ? category_node.replace("node_","") : root_category_id;
		...
		var getCategoryInfo = function()
		{
			...
			var catalog_ajax_url = '{{URL::action('Agriya\Webshoppack\AdminProductCategoryController@postCategoryInfo')}}';
		...
		{
			setAjaxFunctionsCount();
			var attribute_ajax_url = '{{URL::action('Agriya\Webshoppack\AdminCategoryAttributesController@postAttributesInfo')}}';
			var pars = 'category_id='+category_id;
		...
		var attributeOnReadyFunctions = function()
		{
		    ...
		    $(".formBuilderAssignedTable").tableDnD({
		        onDrop: function(table, row) {
					...
					$.ajax({
						type: 'GET',
						url: "{{URL::action('Agriya\Webshoppack\AdminCategoryAttributesController@getAttributesOrder')}}?" + $.tableDnD.serialize()+'&category_id='+category_id,
						data: '',
		...
		var formBuilderAddListRow = function()
		{
			...
			setAjaxFunctionsCount();
			$.post('{{URL::action('Agriya\Webshoppack\AdminCategoryAttributesController@postAdd')}}', pars,  function(data)
			{
		...
		var getCategoryDetails = function()
		{
			...
			var category_ajax_url = "{{URL::action('Agriya\Webshoppack\AdminManageProductCatalogController@postCategoryDetailsBlock')}}";
		...
		var deleteCategory = function()
		{
			...
			$("#dialog-category-delete-confirm").dialog({ title: '{{ trans('webshoppack::admin/manageCategory.delete_category') }}', modal: true,
				buttons: {
					"{{ trans('webshoppack::common.yes') }}": function() {
						$(this).dialog("close");
						setAjaxFunctionsCount();
						var category_ajax_url = '{{URL::action('Agriya\Webshoppack\AdminProductCategoryController@getDeleteCategory')}}';
						...
						$.getJSON(category_ajax_url+pars,  function(data)
						{
							...
							if (data.err)
							{
								...
								$("#dialog-category-err-msg").dialog({ title: '{{ trans('webshoppack::admin/manageCategory.delete_category') }}', modal: true,
							...
							else
							{
								...
								$('#dialog-category-err-msg-content').html('{{ trans('webshoppack::admin/manageCategory.delete-category.delete_category_success_msg') }}');
								$("#dialog-category-err-msg").dialog({ title: '{{ trans('webshoppack::admin/manageCategory.delete_category') }}', modal: true,
									buttons: { "{{ trans('webshoppack::common.ok') }}": function() { $(this).dialog("close"); } }
								});
							}
...
var doAjaxSubmit = function() {
	...
	var formBuilderRemoveListRow = function()
	{
		...
		$("#dialog-attribute-remove-confirm").dialog({ title: "{{ trans('webshoppack::admin/manageCategory.attributes_title') }}", modal: true,
			buttons: {
				"{{ trans('webshoppack::common.yes') }}": function() {
					$(this).dialog("close");

					$.getJSON("{{URL::action('Agriya\Webshoppack\AdminCategoryAttributesController@getDeleteAttributes')}}?attribute_id=" + attribute_id + '&category_id=' + attribute_category,
					{
						beforeSend:function()
						{
							setAjaxFunctionsCount();
					...
					function(data)
					{
						...
						if(!data.err)
						{
							$('#ajaxMsgSuccess').html('{{ trans('webshoppack::admin/manageCategory.attributes_assigned_removed_msg') }}').show().fadeOut(2000);
							$('#ajaxMsgs').hide();
						}
					});
				}, "{{ trans('webshoppack::common.cancel') }}": function() { $(this).dialog("close"); }
