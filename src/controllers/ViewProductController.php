	...
	public function getIndex($slug_url)
	{
		...
		if(count($p_details) > 0)
		{
			...
			if($p_details['product_status'] != 'Ok')
			{
				...
				{
					$alert_msg = "mp_product/viewProduct.draft_or_not_approval";
					$preview_mode = true;
				}
				else
				{
			...
			if($error_msg == '')
			{
				...
				if($p_details['product_status'] == 'Ok')
				{
					$view_cookie = $this->ViewProductService->addProductViewCount($p_details['id']);
					if($view_cookie != '')
					{
				...
				$d_arr['slider_default_img'] = $this->ViewProductService->fetchSliderDefaultImage($p_details['id']);
				$d_arr['slider_preview_img'] = $this->ViewProductService->fetchSliderPreviewImage($p_details['id']);
			...
			$view_url = $this->ViewProductService->getProductViewURL($p_details['id'], $p_details);
	...
	public function addReplyComment()
	{
		...
		if($product_code)
		{
			...
			if ( $validator->passes())
			{
				...
				if(count($p_details) > 0)
				{
					...
					if($return_arr['allow_to_reply'])
					{
						...
						if($thread_id > 0)
						{
							...
							$success_message =  trans("mp_product/viewProduct.reply_message_added_success");
						    $key = "NewReply";
						}
						else
						{
							...
							$success_message =  trans("mp_product/viewProduct.comment_added_success");
							$key = "NewComment";
						}
				...
				{
					$error_message = trans("mp_product/viewProduct.invalid_url_slug");
				}
			}
			else
			{
...
//		echo "<br>thread_id",$thread_id;
    ...
    public function addQuery($p_details)
    {
		...
		$d_arr = $breadcrumb_arr = array();
		$page_title = trans("mp_product/viewProduct.page_title");
		$reply = 0;
		if(Input::has('id'))
		{
	...
	public function changeQueryStatus($p_details)
    {
		...
		if($discussion_details)
		{
			if($status == 'mark_as_private')
			{
				...
				{
					$error_msg = trans("mp_product/viewProduct.private_question_already_marked_as_private_msg");
				}
				else
				{
			...
			elseif($status == 'unmark_from_private')
			{
				...
				{
					$error_msg = trans("mp_product/viewProduct.private_question_already_unmarked_from_private_msg");
				}
				else
				{
			...
			{
				$error_msg = trans("mp_product/viewProduct.private_question_invalid_action");
			}
		...
		if(Input::has('unmark_from_private_submit'))
		{
			...
			if(count($p_details) > 0)
			{
				...
				return Redirect::to($item_url.'?discussion_id='.$discussion_id.'&status=unmark_from_private')
								...
								->with('success_message', trans('mp_product/viewProduct.private_question_unmarked_from_private_success_msg'));
			}
		...
		elseif(Input::has('mark_as_private_submit'))
		{
			...
			if(count($p_details) > 0)
			{
				...
				return Redirect::to($item_url.'?discussion_id='.$discussion_id.'&status=mark_as_private')
								...
								->with('success_message', trans('mp_product/viewProduct.private_question_marked_as_private_success_msg'));
			}
		}
		else
		{
			...
			if(is_numeric($p_id))
			{
				...
				if(count($p_details) > 0)
				{
					...
					if($is_allowed_access)
			    	{
						...
						if($validator->passes())
						{
							$suc_message = trans('mp_product/viewProduct.reply_post_msg');
							$params = '?query=add';
							if($discussion_id)
							{
							...
							else
							{
								...
								$suc_message = trans('mp_product/viewProduct.buyer_note_success');
							}
			    	...
			    	return Redirect::to($item_url.'?query=add')->with('error_message', trans('mp_product/viewProduct.invalid_url_slug'));
				}
			}
		}
	...
	public function postRating()
	{
		...
		{
			echo "error|".trans('mp_product/viewProduct.issue_in_rating');exit;
		}
		$p_arr = Product::whereRaw('product_code = ?', array($inputs['product_code']))->first();
		if(empty($p_arr))
		{
			echo "error|".trans('mp_product/viewProduct.invalid_product');exit;
		}
		if(!isLoggedIn())
    	{
    	...
    	if(!$is_bought)
    	{
			echo "error|".trans('mp_product/viewProduct.rate_after_purchase');exit;
		}
		$inputs['mp_product_id'] = $p_arr['id'];
		$rate_id = $this->ViewProductService->addRating($inputs);
		if($rate_id)
		{
			echo "success|".trans('mp_product/viewProduct.thanks_voting');exit;
		}
		else
		{
			echo "reload|".trans('mp_product/viewProduct.rating_updated');exit;
		}
	...
	public function postLike()
	{
		...
		{
			echo "error|".trans('mp_product/viewProduct.issue_in_like');exit;
		}
		$p_arr = Product::whereRaw('product_code = ?', array($inputs['product_code']))->first();
		if(empty($p_arr))
		{
    	...
    	if($is_liked)
    	{
			echo "error|".trans('mp_product/viewProduct.already_liked');exit;
		}
		...
		{
			echo "reload|".trans('mp_product/viewProduct.thanks_like');exit;
		}
		exit;
	}
	...
	public function getDemo($url_slug)
	{
		...
		if(count($p_arr) > 0)
		{
			$page_title = str_replace('VAR_SCRIPT_NAME', $p_arr['title'], trans('mp_product/viewProductURL.demo_page_title'));
