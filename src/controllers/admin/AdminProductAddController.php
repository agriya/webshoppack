...
class AdminProductAddController extends \BaseController {
	...
	public function postAdd()
	{
		if(\Input::has('add_product'))
		{
			...
			$validator_arr = $this->productAddService->getproductValidation($input_arr);
			$validator = \Validator::make($input_arr, $validator_arr['rules'], $validator_arr['messages']);
			if($validator->passes())
			{
				$add_product = $this->productAddService->addProduct($input_arr);
				if($add_product > 0)
				{
					//To redirect to next tab..
					return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add?id='.$add_product.'&p=price');
				}
			}
			else
			{
				return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add')->with('error_message', trans('webshoppack::common.correct_errors'))->withInput()->withErrors($validator);
			}
		}
		return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add');
	}
			...
			if(isset($input_arr['p']) && $input_arr['p'] == 'download_files')
			{
				if($this->productService->validateDownloadTab($input_arr['id']))
				{
					...
					if($new_tab_key == '')
					{
						...
						$msg = (empty($this->productAddService->alert_message))? '' : trans('webshoppack::product.'.$this->productAddService->alert_message);
						return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add?id='.$input_arr['id'].'&p=status')->with('success_message', $msg);
					}
					else
					{
						//To redirect to next tab..
						return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add?id='.$input_arr['id'].'&p='.$new_tab_key);
					}
				}
				else
				{
					return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add?id='.$input_arr['id'].'&p=download_files')->with('error_message', trans('webshoppack::common.correct_errors'));
				}
			}
			else
			{
				...
				if($validator->passes())
				{
					...
					if($update_product_arr['status'])
					{
						...
						if($new_tab_key == '')
						{
							//If all the completed, then redirect to product list page
							$msg = (empty($this->productAddService->alert_message))? '' : trans('webshoppack::product.'.$this->productAddService->alert_message);
							return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add?id='.$input_arr['id'].'&p=status')->with('success_message', $msg)->with('validate_tab_arr', $validate_tab_arr);
						}
						else
						{
							//To redirect to next tab..
							return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add?id='.$input_arr['id'].'&p='.$new_tab_key)->with('validate_tab_arr', $validate_tab_arr);
						}
					}
					else
					{
						return \Redirect::to(\Config::get('webshoppack::admin_uri').'/add?id='.$input_arr['id'].'&p=status')->with('validate_tab_arr', $validate_tab_arr);
					}
				}
				else
				{
