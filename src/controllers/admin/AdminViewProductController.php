...
//		echo "<br>thread_id",$thread_id;
		...
		if(Input::has('unmark_from_private_submit'))
		{
			...
			if(count($p_details) > 0)
			{
				...
				return Redirect::to($item_url.'?discussion_id='.$discussion_id.'&status=unmark_from_private')
								...
								->with('success_message', trans('webshoppack::viewProduct.private_question_unmarked_from_private_success_msg'));
			}
		...
		elseif(Input::has('mark_as_private_submit'))
		{
			...
			if(count($p_details) > 0)
			{
				...
				return Redirect::to($item_url.'?discussion_id='.$discussion_id.'&status=mark_as_private')
								...
								->with('success_message', trans('webshoppack::viewProduct.private_question_marked_as_private_success_msg'));
			}
		}
		else
		{
			...
			if(is_numeric($p_id))
			{
				...
				if(count($p_details) > 0)
				{
					...
					if($is_allowed_access)
			    	{
						...
						if($validator->passes())
						{
							$suc_message = trans('webshoppack::viewProduct.reply_post_msg');
							$params = '?query=add';
							if($discussion_id)
							{
							...
							else
							{
								...
								$suc_message = trans('webshoppack::viewProduct.buyer_note_success');
							}
			    	...
			    	return Redirect::to($item_url.'?query=add')->with('error_message', trans('webshoppack::viewProduct.invalid_url_slug'));
				}
			}
		}
	...
	public function postRating()
	{
		...
		{
			echo "error|".trans('webshoppack::viewProduct.issue_in_rating');exit;
		}
		$p_arr = Product::whereRaw('product_code = ?', array($inputs['product_code']))->first();
		if(empty($p_arr))
		{
			echo "error|".trans('webshoppack::viewProduct.invalid_product');exit;
		}
		if(!isLoggedIn())
    	{
    	...
    	if(!$is_bought)
    	{
			echo "error|".trans('webshoppack::viewProduct.rate_after_purchase');exit;
		}
		$inputs['mp_product_id'] = $p_arr['id'];
		$rate_id = $this->ViewProductService->addRating($inputs);
		if($rate_id)
		{
			echo "success|".trans('webshoppack::viewProduct.thanks_voting');exit;
		}
		else
		{
			echo "reload|".trans('webshoppack::viewProduct.rating_updated');exit;
		}
	...
	public function postLike()
	{
		...
		{
			echo "error|".trans('webshoppack::viewProduct.issue_in_like');exit;
		}
		$p_arr = Product::whereRaw('product_code = ?', array($inputs['product_code']))->first();
		if(empty($p_arr))
		{
    	...
    	if($is_liked)
    	{
			echo "error|".trans('webshoppack::viewProduct.already_liked');exit;
		}
		...
		{
			echo "reload|".trans('webshoppack::viewProduct.thanks_like');exit;
		}
		exit;
	}
