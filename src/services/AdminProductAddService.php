...
class AdminProductAddService
{
	...
	{
		$this->p_tab_arr = array('basic' => false, 'price' => false, 'meta' => false, 'attribute' => false, 'preview_files' => false, 'download_files' => false, 'status' => false);
	...
	{
		$status_arr = array();
		$product_status_arr = array('Draft', 'Ok', 'Deleted', 'ToActivate', 'NotApproved');
	...
	public function getproductValidation($input_arr, $id = 0, $tab = 'basic', $action = 'add')
    {
		...
		if($tab == 'basic')
		{
			...
			if($action == 'add')
			{
				...
				$user_msg_arr = array('url_slug.is_valid_slug_url' => trans('webshoppack::admin/productAdd.invalid_slug_url'),
										'user_code.min' => trans("webshoppack::admin/productAdd.invalid_user_code"),
										'user_code.is_user_code_exists' => trans("webshoppack::admin/productAdd.invalid_user_code"),
									);
		...
		if($current_tab != '')
		{
			...
			if($new_tab_key == 'attribute' || $new_tab_key == 'download_files')
			{
				...
				if(count($p_details) > 0)
				{
					...
					{
						if(!$this->productService->checkProductHasAttributeTab($p_details['product_category_id']))
						{
