...
class ViewProductService extends ProductService
{
    ...
    {
		$cat_arr = array();
		$q = DB::select('SELECT parent.category_name, parent.seo_category_name FROM mp_product_category AS node, mp_product_category AS parent WHERE node.category_left BETWEEN parent.category_left AND parent.category_right  AND node.id = ? ORDER BY node.category_left;', array($cat_id));
		if(count($q) > 0)
		{
			foreach($q AS $cat)
			{
